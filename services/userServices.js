...
module.exports.getMyProfile = async (serviceData) => {
  ...
  try {
    ...
    console.log(`Something went wrong: service : userService : getMyProfile`);
    throw new Error(error);
  }
};
...
module.exports.updateMyPassword = async (serviceData) => {
  ...
  try {
    ...
    if (isMatched) {
      const newPasswordHash = await bcryptjs.hash(body.password, 10); // Hash the new password
      // Update the password in the database
      const dbResponse = await userModel.findByIdAndUpdate(
        id,
      ...
      if (dbResponse) {
        ...
        response.message = constants.authMessage.PASSWORD_NOT_UPDATED;
        response.errors.error = constants.authMessage.PASSWORD_NOT_UPDATED;
      }
    } else {
      response.errors.error = constants.authMessage.PASSWORD_MISMATCHED;
  ...
  try {
    ...
    if (userResponse) {
      if (userResponse.otp == serviceData.otp) {
        ...
        response.body = { token };
        response.message = "You OTP verified successfully";
        response.status = 200;
      } else {
        response.errors.otp = "You entered an invalid OTP";
        response.message = "You entered an invalid OTP";
...
module.exports.createNewPassword = async (serviceData, userId) => {
  ...
  try {
    ...
    if (dbResponse) {
      ...
      response.errors.error = "Your Password haven't been Created";
      response.message = "Your Password haven't been Created";
    }
